...
## Pipeline
- **Corpus**: 6 hardcoded documents (policies, guides, FAQs, incident reports) covering compliance, onboarding, product, and operations, designed to include two conflicting versions of the same policy (`doc_1`/`doc_2`) to test recency handling.
...
## Structure
```
src/    main.py — single script: ingestion, chunking, embedding, hybrid retrieval, prompt building, generation, and demo queries
```
